...
@router.post("/api/v1/public/register", response_model=User)
def register(new_user: NewUser, db: Session = Depends(get_db)):
    return create_user(db, new_user)


@router.get("/api/v1/public/instrument", response_model=list[Instrument])
...
def get_orderbook_endpoint(
    ...
    return L2OrderBook(
        bid_levels=[{"price": o.price, "qty": o.quantity - o.filled} for o in bids],
        ask_levels=[{"price": o.price, "qty": o.quantity - o.filled} for o in asks]
    )


@router.get("/api/v1/public/transactions/{ticker}", response_model=list[Transaction])
